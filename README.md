...
- **Decoy Vault (duress password)**: While creating the vault you may set an optional second password that opens a completely separate, real-but-harmless vault (its own SQLCipher file and its own random DEK, never held in memory together with the real one). The decoy vault comes pre-seeded with plausible filler items so it never looks empty, and it behaves identically — search, pin, archive, TOTP, per-vault activity log and backups. The decoy password is asked **only once**, during vault creation; no setting to enable, change or remove it ever appears again (managing it requires re-creating the vault). Wrong-password attempts try both keys silently, so the unlock prompt gives no timing or messaging hint that a second vault exists. A "3 wrong attempts" wipe and the full data wipe erase **both** vaults. Note: this defends against password coercion and casual device inspection, not against a privileged forensic adversary specifically looking for this scheme.
